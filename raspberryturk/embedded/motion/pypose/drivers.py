...
class Drivers:
    ...
    def torque_enable(self, dxl_id):
        dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, dxl_id, ADDR_MX_TORQUE_ENABLE,
                                                                       TORQUE_ENABLE)
    ...
    def syncwrite_goal_position(self, vals1, vals2):
        ...
        while 1:
            ...
            vals1[0], vals1[1], dxl1_present_position, vals2[0], vals2[1], dxl2_present_position))
            if (not (abs(vals1[1] - dxl1_present_position) > DXL_MOVING_STATUS_THRESHOLD)) and (
            not (abs(vals2[1] - dxl2_present_position) > DXL_MOVING_STATUS_THRESHOLD)):
                break
